...
func main() {
	...
	mustMapEnv(&svc.logSvcAddr, "LOG_SERVICE_ADDR")
	...
	mustConnGRPC(ctx, &svc.logSvcConn, svc.logSvcAddr)
...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	err = cs.addLog(ctx, req.UserId,orderID.String(),prep.orderItems)
	if err != nil{
		log.Infof("%s", err)
	} else{
		log.Infof("logged order %s", orderID.String())
	}
...
func (cs *checkoutService) addLog(ctx context.Context, userID string, orderID string, cartItems []*pb.OrderItem ) error {
	var items []string
	
	cl := pb.NewLogServiceClient(cs.logSvcConn)
	for _, item := range cartItems {
		items = append(items, item.Item.GetProductId())
	}
	if _,err := cl.AddLog(ctx , &pb.AddLogRequest{UserId: userID, OrderId: orderID , Items: items}); err != nil{
		return fmt.Errorf("failed to log order: %+v", err)
	}
	return nil
}

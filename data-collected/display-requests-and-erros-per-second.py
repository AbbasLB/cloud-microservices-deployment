...
# Function to plot Requests/s and Failures/s with the specified time offset
def plot_requests_failures(file_path, time_offset):
    ...
    # Plot Failures/s without markers on the secondary y-axis
    plt.twinx()
    plt.plot(df['Timestamp'], df['Failures/s'], color='red', label='Failures/s')

    # Set y-axis labels and legends
    plt.ylabel('Requests/s & Failures/s')
    plt.xlabel('Time')
    plt.title('Graph for Requests/s and Failures/s')
...
def plot_graphs_in_directory(directory_path, time_offset):
    # Loop through each CSV file in the directory and plot graph for Requests/s and Failures/s
    for filename in os.listdir(directory_path):
        if filename.endswith(".csv"):
            file_path = os.path.join(directory_path, filename)
            plot_requests_failures(file_path, time_offset)

# Parse command-line arguments
parser = argparse.ArgumentParser(description='Plot Requests/s and Failures/s from CSV files.')
